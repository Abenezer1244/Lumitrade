...
class StateManager:
    ...
    async def restore(self) -> None:
        ...
        # 2. Fetch account balance — use PAPER_BALANCE in paper mode so the
        # dashboard shows a realistic demo balance instead of the live account's
        # actual cash (which may be $3.50 during the demo week).
        try:
    ...
    async def persist_loop(self, oanda_client=None) -> None:
        ...
        while not self._shutdown_event.is_set():
            ...
            # In paper mode use PAPER_BALANCE — avoids showing the real $3.50
            # live account balance on the dashboard during demo week.
            # In live mode fetch from OANDA; track failures (stale balance means
            # position sizer computes risk on outdated equity == real money risk).
            if self._oanda:
                try:
                    if self._config.effective_trading_mode() != "LIVE":
                        paper_bal = str(self._config.paper_balance)
